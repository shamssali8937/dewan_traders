...
export default function NewProductPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Ensure absolute path resolution or fallback
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              ...
              <input {...register('name')} placeholder="e.g. Sargodha Kinnow Mandarin" className="w-full px-3 py-2.5 bg-white rounded-xl text-xs text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-1 focus:ring-primary/40 border border-slate-200" />
              {errors.name && <p className="text-[10px] text-red-500 mt-1">{errors.name.message}</p>}
            </div>

            <div>
              <label className="text-[10px] text-slate-500 uppercase font-semibold mb-1.5 block">Product SKU *</label>
              <input {...register('sku')} placeholder="e.g. DT-FR-001" className="w-full px-3 py-2.5 bg-white rounded-xl text-xs text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-1 focus:ring-primary/40 border border-slate-200" />
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            <div>
              ...
              <input {...register('price')} placeholder="e.g. 80.00" className="w-full px-3 py-2.5 bg-white rounded-xl text-xs text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-1 focus:ring-primary/40 border border-slate-200" />
            ...
            <div>
              ...
              <input {...register('unit')} placeholder="e.g. kg, piece, set" className="w-full px-3 py-2.5 bg-white rounded-xl text-xs text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-1 focus:ring-primary/40 border border-slate-200" />
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      <ImageIcon size={13} /> Choose Image File
                    </>
                  )}
                </label>
              </div>
